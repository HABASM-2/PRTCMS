  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: {
        ...
        managerTag: body.managerTag ?? null,
        roles: {
        ...
        UserOrgUnit: {
          deleteMany: {},
          create: Array.isArray(orgUnitIds)
          ? orgUnitIds
              .filter((id: any) => !isNaN(Number(id))) // remove invalid strings like "org-2"
              .map((id: any) => ({
                orgUnit: { connect: { id: Number(id) } },
              }))
          : [],
        },
      },
