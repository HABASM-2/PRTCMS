...
interface Project {
  ...
  submittedBy?: { id: number; fullName: string; email: string } | null;
}
...
export default function ProjectsPage({ userId, roles }: Props) {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/projects/all?${params.toString()}`);
                    ...
                    <React.Fragment key={project.id}>
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              project.remaining >= 0 ? "default" : "destructive"
                            }
                          >
                            {project.remaining >= 0
                              ? project.remaining.toFixed(2)
                              : `Debt ${Math.abs(project.remaining).toFixed(
                                  2
                                )}`}
                          </Badge>
                        </TableCell>
                        <TableCell>{project.orgUnit?.name ?? "-"}</TableCell>
                        <TableCell>
