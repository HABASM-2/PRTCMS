    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `/api/orgunits/org-units/id/?id=${organisationId}${userId ? `&userId=${userId}` : ''}`
        )
  ...
  const renderTree = (units: OrgUnit[]) => {
    return (
        ...
        {units.map((unit) => (
            ...
            {Array.isArray(unit.children) && unit.children.length > 0 && renderTree(unit.children)}
            </li>
        ))}
        </ul>
    );
  ...
  return (
    <div className="mt-2 max-h-64 overflow-y-auto border rounded p-2">
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground">No organisation units found.</p>
      ) : (
        renderTree(tree)
      )}
    </div>
